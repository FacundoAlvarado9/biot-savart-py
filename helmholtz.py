...
Original file is located at
    https://colab.research.google.com/drive/1JoxOB_9ZlTyGI7zlFdtnslcGIttHJn1q
...
"""En este caso particular, trabajaremos con las Bobinas de Helmholtz, representadas por dos espiras circulares de radio 1, separadas por una distancia de R=1. Ambas bobinas estarán dibujadas en el plano xy, estando una en Z=0.5 y otra en Z=-0.5"""

l = [smp.Matrix([smp.cos(2*np.pi*t), smp.sin(2*np.pi*t), 0.5]),
     smp.Matrix([smp.cos(2*np.pi*t), smp.sin(2*np.pi*t), -0.5])] #Espira circular
I = np.full(6, 10) # Todas con corriente 5A en el mismo sentido
...
sep almacena, para cada línea parametrizada, el vector corriespondiente a la posición de la misma en la lista original. Esto es, si la espira circular está en l[1], entonces el vector separación para esta línea será sep[1]
"""
...
"""Por ejemplo, y volviendo a la espira circular, para un punto (x,y,z) el vector $\vec{r} - \vec{l}$ luce de la siguiente manera
...
"""Por ejemplo, para la espira circular:
...
def B(valor_i, dBxdt, dBydt, dBzdt, x, y, z):
    constantes = (mu0*valor_i)/(2*np.pi)
    Bx = np.zeros_like(x) + constantes*quad_vec(dBxdt, 0, 1, args=(x, y, z))[0]
    By = np.zeros_like(x) + constantes*quad_vec(dBydt, 0, 1, args=(x, y, z))[0]
    Bz = np.zeros_like(x) + constantes*quad_vec(dBzdt, 0, 1, args=(x, y, z))[0]
...
for i, lt in enumerate(l):
  B_total = B_total + B(I[i], dbx[i], dby[i], dbz[i], xv, yv, zv)
